...
return (
    <div className=' bg-slate-800 w-full pb-12 '>
        <div className='grid grid-cols-3 pl-9 card'>
            {product.map((product)=>(
            <div key={product.id} className="w-76 md:w-80 mt-10 ml-12 pb-5 rounded hover:scale-105  transform transition duration-300 bg-white">
                <img className='w-72 md:w-80 h-52 object-cover pic' src={product.pic} alt='fail' />
            ...
            {
                <button onClick={()=>addcart(product)} className='bg-blue-700 text-white font-bold py-2 w-72 rounded hover:bg-blue-900 transition duration-300 btn'>Add to cart</button>
            }
            </div>
        </div>
        ))}
